 ...
 * @returns true if DB token was persisted and cookies were set; false otherwise.
 */
export async function attachSingleDeviceSessionCookies(
    res: NextResponse,
    email: string
): Promise<boolean> {
    ...
    if (error || !data) {
        console.error("[single-session] failed to persist session token", error ?? "no matching student row")
        return false
    }
