...
export async function POST(req: Request) {
    try {
        ...
        const sessionAttached = await attachSingleDeviceSessionCookies(res, email)
        if (!sessionAttached) {
            console.error("[get-session] session_token was not persisted", { email })
            return NextResponse.json(
                {
                    ok: false,
                    error: "Failed to create session cookies (session_token not persisted)",
                },
                { status: 500 }
            )
        }
